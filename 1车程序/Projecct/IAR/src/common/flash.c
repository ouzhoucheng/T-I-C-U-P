...
//-------------------------------------------------------------------------------------------------------------------
uint8_t flash_check(uint32_t sector_num, uint32_t page_num)
{
  ...
  {
    if(0xFFFFFFFF != flash_read(sector_num, page_num * FLASH_PAGE_SIZE / 4 + i,uint32_t))
...
//-------------------------------------------------------------------------------------------------------------------
uint8_t flash_erase_sector(uint32_t sector_num)
{
  status_t status;
...
//  Sample usage:               
//-------------------------------------------------------------------------------------------------------------------
uint8_t flash_page_program(uint32_t sector_num, uint32_t page_num, const uint32_t *buf, uint8_t len)
{
  uint16_t i = 0;
  status_t status;
...
//  Sample usage:               
//-------------------------------------------------------------------------------------------------------------------
void flash_read_page(uint32_t sector_num, uint32_t page_num, uint32_t *buf, uint8_t len)
{
  assert(FLASH_SECTOR_NUM > sector_num);                //校验参数合法性
  assert(FLASH_SECTOR_PAGE_NUM > page_num);             //校验参数合法性
  assert(FLASH_PAGE_SIZE >= (len * 4));                   //校验参数合法性
  ...
  for(uint8_t i = 0; i < len; i ++)
    *(buf + i) = flash_read(sector_num, page_num * FLASH_PAGE_SIZE + i,uint32_t);
