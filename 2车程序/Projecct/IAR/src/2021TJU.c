...
void example_uart_callback(LPUART_Type *base, lpuart_handle_t *handle, status_t status, void *userData)
{
    ...
    else if(mycar.status == 1 && uart_data == 0x12)  // 左转
    {
        // watch.NextDir = 3;
        mycar.status = 3;
        watch.turn90 = 1;
    }
    else if(mycar.status == 1 && uart_data == 0x13)  // 右转
    {
        // watch.NextDir = 4;
        mycar.status = 3;
        watch.turn90 = 2;
...
void uart1_callback(LPUART_Type *base, lpuart_handle_t *handle, status_t status, void *userData)
{
    if (kStatus_LPUART_RxIdle == status)
    {
        uart1_data = uart1_rx_buffer; //将数据取出
    }
    ips200_showint32(25, 9, uart1_data, 5);

    if(mycar.status == 4 && uart1_data == 0x55)    // 2号房起步
    {
        mycar.status = 6;
        watch.Temp = 1; // 从临时停车点继续出发
